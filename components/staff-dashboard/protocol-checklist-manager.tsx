...
interface ProtocolChecklistManagerProps {
    bookingId: string;
    showAdd?: boolean;
    showDelete?: boolean;
}

export function ProtocolChecklistManager({ 
    bookingId, 
    showAdd = true, 
    showDelete = true 
}: ProtocolChecklistManagerProps) {
    ...
    // Fetch Items - pointing to protocol-checklist
    const { data: items = [], isLoading, isFetching, isError } = useQuery<ChecklistItem[]>({
        queryKey: ['protocol-checklist', bookingId],
        queryFn: async () => {
            const res = await fetch(`/api/bookings/${bookingId}/protocol-checklist`);
            if (!res.ok) throw new Error('Failed to fetch protocol checklist');
            return res.json();
        }
    });
    ...
    const toggleCompleteMutation = useMutation({
        mutationFn: async ({ id, is_completed }: { id: string; is_completed: boolean }) => {
            const res = await fetch(`/api/bookings/${bookingId}/protocol-checklist/${id}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
            ...
            <ScrollArea className="h-[300px] pr-4">
                {isLoading || isRefreshing ? (
                    ...
                    <div className="text-center p-4 text-destructive text-sm font-medium">
                        {t('errorFetching') || 'Error loading protocol checklist'}
                    </div>
                ) : items.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {items.map((item) => (
                            <div key={item.id} className="group border p-3 space-y-2 bg-card hover:bg-card transition-colors" style={{borderRadius: '0.5rem'}}>
