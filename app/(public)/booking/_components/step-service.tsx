...
export function StepService() {
    ...
    const validatePolicyFields = (): Record<string, string> => {
        ...
        for (const field of selectedService.policyFields) {
            ...
            switch (field.field_type) {
                case 'checkbox':
                    if (value !== true) errors[field.id] = 'Bevestig dit beleid om door te gaan.';
                    break;
                case 'multi_choice':
                    if (!Array.isArray(value) || value.length === 0) errors[field.id] = 'Maak minstens één keuze.';
                    break;
                case 'number_input':
                    if (typeof value !== 'number' || Number.isNaN(value)) errors[field.id] = 'Voer een geldig nummer in.';
                    break;
                case 'date_time':
                    if (typeof value !== 'string' || value.trim() === '') errors[field.id] = 'Selecteer een datum en tijd.';
                    break;
                case 'file_upload':
                ...
                case 'text_input':
                default:
                    if (typeof value !== 'string' || value.trim() === '') errors[field.id] = 'Dit veld is verplicht.';
                    break;
            }
        }
        return errors;
                ...
                return (
                    <div key={field.id} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            {field.choices.length === 0 && <p className="text-sm text-gray-500">Geen opties beschikbaar.</p>}
            ...
            case 'file_upload': {
                return (
                    <div key={field.id} className="space-y-2">
                        ...
                        {description}
                        <p className="text-sm text-amber-600">Uploaden via dit formulier wordt binnenkort ondersteund. Neem contact op met de kliniek als dit verplicht is.</p>
                        {error && <p className="text-xs text-red-600">{error}</p>}
                    </div>
                );
            }
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-2xl font-poppins font-bold text-gray-900">Select service</h2>
                <p className="text-sm text-gray-600">Choose a service to continue.</p>
            </div>
            ...
            {selectedService?.policyFields.length ? (
                <div className="space-y-4 p-4 bg-white animate-in fade-in slide-in-from-top-4 duration-500 ease-in-out" style={{ borderRadius: '0.2rem' }}>
                    <h3 className="text-sm font-bold tracking-wide text-gray-600">Servicebeleid</h3>
                    <div className="space-y-4">
