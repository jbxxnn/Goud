...
export function StepReview() {
    ...
    const {
        selectedService, date, selectedSlot, selectedAddOnItems,
    ...
    const handleLogin = async (emailForLogin: string) => {
        setErrorMsg('');
        if (!emailForLogin) { setErrorMsg('E-mailadres is verplicht'); return; }
        if (!password) { setErrorMsg('Voer uw wachtwoord in om door te gaan'); return; }
        ...
        try {
            ...
            if (loginErr) throw new Error(loginErr.message || 'Inloggen mislukt');
            if (!loginData?.session) throw new Error('Inloggen mislukt: geen sessie aangemaakt');
            ...
            try {
                ...
                // In the context, we should probably expose a way to re-fetch/update defaults if we want to be 100% correct,
                // but the context auto-reloads defaults on login state change so we might be fine relying on that effect.
                // However, for immediate feedback we can leverage the context's setContactDefaults if we exported it, 
                // OR we just rely on the existing effect in BookingProvider that watches [isLoggedIn, clientEmail].
            } catch (e) {
                console.error('Error fetching user details after login:', e);
            }
        } catch (e: any) {
            setErrorMsg(e?.message || 'Inloggen mislukt');
        } finally {
            setFinalizing(false);
        }
    };
    ...
    const handleBookingSubmit = async (values: BookingContactInput) => {
        try {
            ...
            setErrorMsg(e?.message || 'Boeking is niet gelukt');
        } finally {
            setFinalizing(false);
        }
    };
