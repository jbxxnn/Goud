            ...
            {finalizing && (
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/80 backdrop-blur-sm rounded-lg animate-in fade-in duration-300">
                    <div className="flex flex-col items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-primary animate-pulse">Processing your details...</span>
                    </div>
                </div>
            )}
            <form id="checkout-form" className="space-y-6" onSubmit={handleSubmit(onSubmit)} noValidate>
                <div className="grid md:grid-cols-2 gap-4">
                    <div className="md:col-span-2 space-y-2">
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">E-mailadres</label>
                        <div className="relative">
                            <Input
                                type="email"
                                readOnly={isLoggedIn}
                        ...
                        {emailChecked?.exists === true && !isLoggedIn && (
                            <div className="absolute top-0 right-0 p-1 bg-blue-50 text-blue-700 text-[10px] rounded-full border border-blue-200 font-medium">
                                Account gevonden. Log in om door te gaan.
                            </div>
                        )}
                        {emailChecked?.exists === true && isLoggedIn && (
                    ...
                    {showLoginForm && (
                        <div className="md:col-span-2 space-y-4 pt-2">
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Wachtwoord</label>
                                <Input
                                    type="password"
                    ...
                    {showDetailsForm && (
                        <>
                                ...
                                <div className="space-y-4 col-span-2 pt-2 pb-2">
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <Label htmlFor="booking-for-client-mode" className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1 mb-0">
                                            Deze boeking is voor een klant
                                        </Label>
                                    </div>
                                    {isBookingForClient && (
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Voornaam</label>
                                <Input
                                    className="h-12 rounded-xl border-gray-200 bg-gray-50/50 hover:bg-white focus:bg-white transition-all duration-200"
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Achternaam</label>
                                <Input
                                    className="h-12 rounded-xl border-gray-200 bg-gray-50/50 hover:bg-white focus:bg-white transition-all duration-200"
                            ...
                            <div className="space-y-2 col-span-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Telefoon</label>
                                <Input
                                    className="h-12 rounded-xl border-gray-200 bg-gray-50/50 hover:bg-white focus:bg-white transition-all duration-200"
                            ...
                            <div className="col-span-2 grid grid-cols-2 gap-4">
                                <div className="col-span-2 md:col-span-1 space-y-2">
                                    <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Straatnaam</label>
                                    <Input placeholder="Straat" className="h-12 rounded-xl border-gray-200 bg-gray-50/50 hover:bg-white focus:bg-white transition-all duration-200" {...streetNameField} />
                                    {errors.streetName && <div className="text-xs text-red-600 font-medium ml-1">{errors.streetName.message}</div>}
                                </div>
                                <div className="col-span-2 md:col-span-1 space-y-2">
                                    <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Huisnummer</label>
                                    <Input placeholder="Nr" className="h-12 rounded-xl border-gray-200 bg-gray-50/50 hover:bg-white focus:bg-white transition-all duration-200" {...houseNumberField} />
                                    {errors.houseNumber && <div className="text-xs text-red-600 font-medium ml-1">{errors.houseNumber.message}</div>}
                                </div>
                                <div className="col-span-1 space-y-2">
                                    <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Postcode</label>
                                    <Input placeholder="1234 AB" className="h-12 rounded-xl border-gray-200 bg-gray-50/50 hover:bg-white focus:bg-white transition-all duration-200" {...postalCodeField} />
                                    {errors.postalCode && <div className="text-xs text-red-600 font-medium ml-1">{errors.postalCode.message}</div>}
                                </div>
                                <div className="col-span-1 space-y-2">
                                    <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Stad</label>
                                    <Input placeholder="Stad" className="h-12 rounded-xl border-gray-200 bg-gray-50/50 hover:bg-white focus:bg-white transition-all duration-200" {...cityField} />
                            ...
                            {/* Dates */}
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Uitgerekende datum</label>
                                <Controller
                                    control={control}
                                    name="dueDate"
                                    render={({ field }) => (
                                        <Popover open={isDueDateOpen} onOpenChange={setIsDueDateOpen}>
                                            <PopoverTrigger asChild>
                                                ...
                                                >
                                                    ...
                                                    {field.value ? format(new Date(field.value), "P") : <span>Selecteer datum</span>}
                                                </Button>
                                            </PopoverTrigger>
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Geboortedatum</label>
                                <Controller
                                    control={control}
                                    name="birthDate"
                                    render={({ field }) => (
                            ...
                            <div className="md:col-span-2 space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Je eigen verloskundige</label>
                                <Select
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        {midwives.map((m) => {
                                            ...
                                            return <SelectItem key={m.id} value={m.id}>{label || 'Onbekende verloskundige'}</SelectItem>;
                                        })}
                                        <SelectItem value="other">Anders / Niet in lijst</SelectItem>
                                    </SelectContent>
                                </Select>
                            ...
                            <div className="md:col-span-2 space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">Opmerkingen</label>
                                <textarea
                                    ...
                                    placeholder="Bijzonderheden of vragen? (Optioneel)"
                                    {...notesField}
                                />
